...
bool MIRFinancialLayer::init(KTPauseLayerDelegate *delegate)
{
    ...
    __Array *array = (__Array *)root->objectForKey("financial");
    ...
    for (int i = 0; i < array->count(); i++) {
        ...
        m->release();
        m->update(InvestmentType::FINANCIAL, array->count() - 1 - i);
    ...
    auto titleLabel = Label::createWithTTF("金融投资", TITLE_FONT, 30);
    titleLabel->setPosition(Point(winSize.width/2, header->getPositionY() + 70));
    addChild(titleLabel);
    
    auto subTitle = Label::createWithTTF("（提高每秒收益，需开启游戏）", TITLE_FONT, 26);
 ...
 void MIRFinancialLayer::tableCellHighlight(TableView* table, TableViewCell* cell)
{
 ...
 void MIRFinancialLayer::tableCellUnhighlight(TableView* table, TableViewCell* cell)
{
    MIRTableViewCell *c = (MIRTableViewCell *)cell;
    if (!c->getTouchEnable()) {
        return;
 ...
 Size MIRFinancialLayer::tableCellSizeForIndex(TableView *table, ssize_t idx)
{
    Size s;
    //adapt
    if (IS_IPAD) {
 ...
 TableViewCell* MIRFinancialLayer::tableCellAtIndex(TableView *table, ssize_t idx)
{
    MIRTableViewCell *cell = (MIRTableViewCell *)table->dequeueCell();
    if (!cell) {
        cell = new MIRTableViewCell(InvestmentType::FINANCIAL);
        cell->autorelease();
    }
...
void MIRFinancialLayer::tableCellTouched(TableView* table, TableViewCell* cell)
{
 ...
 ssize_t MIRFinancialLayer::numberOfCellsInTableView(TableView *table)
{
    return _dataArray.size();
}

MIRFinancialLayer::MIRFinancialLayer()
: _mirDelegate(NULL)
{
